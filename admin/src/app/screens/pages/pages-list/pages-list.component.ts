...
import { Page, InputMetatdata, SchemaMetadata } from '@cx/shared/types';
...
export class PagesListComponent {
  ...
  @Output() onEditContent = new EventEmitter<{
    dataId: string;
    templateId: string;
  }>();
  @Output() onCreateContent = new EventEmitter<{
    templateId: string;
    indexes: number[];
  }>();
  @Output() onDeleteContent = new EventEmitter<{
    dataId: string;
    indexes: number[];
  }>();
  ...
  editContent({
    ...
    if (dataId.length && templateId.length) {
      ...
      UIkit.modal.confirm(`There is no content yet. Create one?`).then(
        () => {
   ...
   */
  createContent(
    { templateId, indexes }: { templateId: string; indexes: number[] },
    index: number
  ): void {
   ...
   */
  deleteContent(
    { dataId, indexes }: { dataId: string; indexes: number[] },
    index: number
  ): void {
    ...
    UIkit.modal.confirm(`Delete page content. Are you sure?`).then(
      () => {
  ...
  addSubPage(index: number): void {
    ...
    this.pages[index].sub.push({ ...this.schemaMetadata.add[1] });
  }
  ...
  removeFirstAdd(schemaMetadata: SchemaMetadata): SchemaMetadata {
    ...
    deeperSchemaMetadata.add = this.sharedService.removeArrayItem(
      deeperSchemaMetadata.add,
      0
    );
