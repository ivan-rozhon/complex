...
import { ContentEditableDirective } from '@cx/shared/directives';

import { MediaSelectors } from '@cx/state/media';

import { Image, ImageData } from '@cx/shared/types';
...
export class EditorComponent implements OnInit {
  ...
  @ViewChild('cxContentEditable')
  contentEditableElementRef: ContentEditableDirective;

  constructor(private store: Store<RootState>) {}

  ngOnInit(): void {
    // images store stream
    this.images$ = this.store.select(MediaSelectors.selectImages);
  }
  ...
  insertImage(imageData: ImageData): void {
    ...
    const contentEditableNativeEl = this.contentEditableElementRef.elementRef
      .nativeElement;
  ...
  editTable(type: string, remove?: boolean): void {
    ...
    // TODO(IRO): "as any" is a temporary fix
    const selectedRow = $(
    ...
    const selectedCellColspan = selectedCell.attr('colspan')
      ? Number(selectedCell.attr('colspan'))
      : null;
    const selectedCellRowspan = selectedCell.attr('rowspan')
      ? Number(selectedCell.attr('rowspan'))
      : null;
    ...
    // https://goo.gl/f757fp
    const columnsCount = [].reduce.call(
      firstRow.find('td'),
      (p, c) => {
    ...
    switch (type) {
      ...
      case 'row': {
        if (remove) {
          ...
          const removedRow =
            selectedTableBody.get(0) && rowsCount > 1
        ...
        const insertedRow = selectedTableBody.get(0)
          ? selectedTableBody.get(0).insertRow(-1)
          : null;
      ...
      case 'rowspan': {
        ...
        if (selectedCellColspan && selectedCellColspan > 1) {
          return;
        }
        ...
        const nextRow = selectedCellRowspan
          ? // if cell has rowspan get next row after 'rowspanned' row
            selectedTableBody
              .find('tr')
              .eq(selectedRowIndex + selectedCellRowspan)
          : selectedRow.next();
        ...
        if (remove) {
          ...
          if (!selectedCellRowspan || selectedCellRowspan < 2) {
            return;
          }

          // row to insert cell
          const nextRowToInsert = selectedTableBody
            .find('tr')
            .eq(selectedRowIndex + selectedCellRowspan - 1);
          ...
          nextRow.find('td').each((index) => {
            ...
            if (nextCellToInsertPosition <= selectedCellPosition) {
              ...
              nextCellToInsertPosition += currentCellColspan
                ? currentCellColspan
                : 1;
            }
          });
          ...
          selectedCell.attr('rowspan', selectedCellRowspan - 1);

          // colspan of updated selected cell
          const actualCellRowspan = selectedCell.attr('rowspan')
            ? Number(selectedCell.attr('rowspan'))
            : null;
        ...
        const nextCellColspan = nextCell.attr('colspan')
          ? Number(nextCell.attr('colspan'))
          : null;
        const nextCellRowspan = nextCell.attr('rowspan')
          ? Number(nextCell.attr('rowspan'))
          : null;
        ...
        if (nextCellRowspan && nextCellRowspan > 1) {
          ...
          selectedCell.attr(
            'rowspan',
            selectedCellRowspan
              ? selectedCellRowspan + nextCellRowspan
              : nextCellRowspan + 1
          );
        } else {
        ...
        if (remove) {
          ...
          selectedTableBody.find('tr').each((index) => {
            ...
            if (colspan && colspan > 1) {
              ...
              const removedCell =
                currentRow.get(0) && !rowspan
                  ? currentRow.get(0).deleteCell(-1)
                  : null;
      ...
      case 'colspan': {
        ...
        if (selectedCellRowspan && selectedCellRowspan > 1) {
          return;
        }

        if (remove) {
          // exit if selected cell has no cellspan
          if (!selectedCellColspan || selectedCellColspan < 2) {
            return;
          }
          ...
          selectedCell.attr('colspan', selectedCellColspan - 1);

          // colspan of updated selected cell
          const actualCellColspan = selectedCell.attr('colspan')
            ? Number(selectedCell.attr('colspan'))
            : null;
        ...
        // check if next cell exists
        if (!nextCell || !nextCell.get().length) {
          return;
        }
        ...
        if (nextCellColspan && nextCellColspan > 1) {
          ...
          selectedCell.attr(
            'colspan',
            selectedCellColspan
              ? selectedCellColspan + nextCellColspan
              : nextCellColspan + 1
          );
        } else {
